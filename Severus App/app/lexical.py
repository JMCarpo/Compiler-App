...
deli_reserved = {"sep(:)": ["default"], "open_array": [
    "array"], "array_delim": ["]"], "value_delim": ["true", "false"], "terminator": ["break"], "grouping": [")"], }
# ----------------------------------------------------------------------------------------------------------
            ...
            if tmptype[i] in RES_WORDS or tmptype[i] in RES_SYMBOLS:  # or tmptype[i] == "comment"
                if tmpvalue[i] == "fs":
                    if tmptype[i + 1] == "stng_literal":
                        ...
                        self.value.append(tmpvalue[i])
                        self.error.append(  
                            f'Lexical Error on Ln {tmpline[i]}: "{str(tmpvalue[i + 1])[0]}" is not a valid delimiter for {tmptype[i]} '
                        )
                        self.line.append(tmpline[i])
                        self.column.append(tmpcolumn[i])
                        break
                ...
                #         True
                #         if tmptype[i + 1] == ("stng_literal") or tmptype[i + 1] == ("char_literal")
                ...
                if tmpvalue[i + 1] in rdefinition["id_delim"]:
                    ...
                    self.column.append(tmpcolumn[i])
                    break
            elif tmptype[i] == "stng_literal" or tmptype[i] == "char_literal":
                if tmpvalue[i + 1] in rdefinition["id_delim"]:
                    self.type.append(tmptype[i])
                    self.value.append(tmpvalue[i])
                    self.error.append(tmperror[i])
